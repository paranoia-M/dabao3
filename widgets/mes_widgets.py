...
class PacingGauge(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.value = 0 # 0-1, 0.5 is target
        self.takt_time = 10.0 # seconds

    def set_value(self, cycle_time):
        # 将 cycle_time 转换为 0-1 的值，0.5为目标
    ...
    def paintEvent(self, event):
        ...
        pen.setColor(QColor("#D32F2F")); painter.setPen(pen) # Red
        painter.drawArc(-100, -100, 200, 200, 0 * 16, -60 * 16)
        
        pen.setColor(QColor("#FFC107")); painter.setPen(pen) # Yellow
        painter.drawArc(-100, -100, 200, 200, -60 * 16, -60 * 16)

        pen.setColor(QColor("#4CAF50")); painter.setPen(pen) # Green
        painter.drawArc(-100, -100, 200, 200, -120 * 16, -60 * 16)
        painter.restore()
